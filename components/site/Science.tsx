...
export default function Science() {
  return (
    <section className="border-y border-line bg-ink py-16 text-white md:py-20">
      <div className="mx-auto max-w-[1140px] px-5">
        <div className="mx-auto max-w-2xl text-center">
          <p className="eyebrow text-teal">The science</p>
          <h2 className="mt-3 text-[clamp(26px,3.2vw,38px)] font-medium">
            Medical science hasn&apos;t stopped evolving. Neither should your options.
          </h2>
          <p className="mt-3 text-[#bcd0e6]">
        ...
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {points.map((p) => (
            ...
            >
              <p.icon className="h-7 w-7 text-teal" />
              <h3 className="mt-4 text-lg font-medium text-white">{p.title}</h3>
              <p className="mt-2 text-[15px] leading-relaxed text-[#bcd0e6]">{p.body}</p>
            </div>
          ))}
        </div>
      </div>
